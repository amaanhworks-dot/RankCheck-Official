...
export default function AimTest() {
  ...
  // ⭐ Add ref for combo sound debounce
  ...
  const captureFinalStats = () => {
    // ⭐ Only play end sound if test was actually running
  ...
  const createExplosion = (x: number, y: number, count: number = 40, isAce: boolean = false) => {
    // ⭐ REMOVED SOUNDS — they're now handled in handleCanvasClick
  ...
  const handleCanvasClick = (e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    for (let i = currentTargets.length - 1; i >= 0; i--) {
      ...
      if (distance <= target.radius) {
        ...
        // ⭐ Combo sound with debounce
        if (newCombo === 3 || newCombo === 6 || newCombo === 10) {
          const now = Date.now();
          if (now - lastComboSoundRef.current > 200) {
            lastComboSoundRef.current = now;
            playSound('combo');
          }
        }
